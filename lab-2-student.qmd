...
glimpse(surveys)
```
Some of the variables are doubles, while others are characters. The `date` column is a date type.
...
# Source: https://ggplot2.tidyverse.org/reference/labs.html
...
ggplot(data = surveys) +
  ...
  labs(x = "Weight (g)",
       y = "",
       title = "Relationship between Weight and Hindfoot Length in Animal Species",
...
ggplot(data = surveys,
  ...
  labs(
    ...
    y = "",
    subtitle = "Weight (g)",
    title = "Weight Distribution Across Species from Portal, Arizona"
  )
```
...
# Source: https://www.geeksforgeeks.org/when-to-use-aov-vs-anova-in-r/
...
Since the p-value = __, which is significantly less than 0.05, we reject the null hypothesis that the population mean weight is the same between all fourteen roden species. Thus, there is strong evidence to suggest that at least one rodent species has a different population mean weight in comparison to the others.
